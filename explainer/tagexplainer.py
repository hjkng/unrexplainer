...
import torch.nn.functional as F
import torch.nn as nn
from explainer.tage_utils import JSE_loss, NCE_loss
...
class TAGExplainer(nn.Module):
	...
	def explain(self, data: Data, embed: Tensor, condition: Tensor,
		...
		if 'Info' in str(self.model):
 			embed, _, _ = self.model(data.x, data.edge_index)           
		else:
	...
	def train_explainer_node(self, data, batch_size=128, lr=0.001, epochs=10):
		""" training the explanation network by gradient descent(GD) using Adam optimizer """
		optimizer = Adam(self.explainer.parameters(), lr=lr)
			# train the mask generator
		for epoch in range(epochs):
			...
			with torch.no_grad():
				all_embeds = self.model(data.x, data.edge_index)
			...
			for node_batch in pbar:
				cond = self.__rand_cond__(1)
				embeds = torch.tensor(all_embeds[node_batch]).to(self.device)
				pruned_embeds = []
				masks = []
				for node_idx in node_batch:
					subgraph, subset = self.get_subgraph(node_idx=node_idx, data=data)
	...
	def __edge_mask_to_node__(self, data, edge_mask, top_k):
		...
		hard_mask = (edge_mask > threshold).cpu() # top_k+1번째 값이 임계값이 됨.
		edge_idx_list = torch.where(hard_mask == 1)[0] # 중요한 edge_idx

		selected_nodes = []
		edge_index = data.edge_index.cpu().numpy()
		for edge_idx in edge_idx_list:
			selected_nodes += [edge_index[0][edge_idx], edge_index[1][edge_idx]]
		selected_nodes = list(set(selected_nodes)) # 중요한 노드들
		maskout_nodes = [node for node in range(data.x.shape[0]) if node not in selected_nodes] # 안중요
		node_mask_out = torch.zeros(data.num_nodes).type(torch.float32).to(self.device)
		node_mask_out[maskout_nodes] = 1.0 # 안중요만 1.0
        
		maskin_nodes = [node for node in range(data.x.shape[0]) if node in selected_nodes]
		node_mask_in = torch.zeros(data.num_nodes).type(torch.float32).to(self.device)
		node_mask_in[maskin_nodes] = 1.0  # 중요만 1.0 
		return node_mask_out, node_mask_in

	def __edge_mask_to_edgeidx(self, data, edge_mask, top_k):
		threshold = float(edge_mask.reshape(-1).sort(descending=True).values[min(top_k, edge_mask.shape[0]-1)])
		hard_mask = (edge_mask > threshold).cpu() # top_k+1번째 값이 임계값이 됨.
		edge_idx_list = torch.where(hard_mask == 1)[0] # 중요한 edge_idx
	...
	def forward(self, data: Data, **kwargs):
		...
		if node_idx is not None:
			if 'Info' in str(self.model):
				with torch.no_grad():
					node_embed, _, _ = self.model(data.x, data.edge_index)
			else:
				with torch.no_grad():
					node_embed = self.model(data.x, data.edge_index)            
		...
		if self.explain_graph:
			...
			node_mask, node_mask_in = self.__edge_mask_to_node__(data, edge_mask, top_k)
			...
			cond = self.__rand_cond__(1) ### newly added--we want to explain it without downstream tasks
			_, edge_mask, log = self.explain(subgraph, node_embed[subset], condition=cond,tmp=1.0, training=False, node_idx=new_node_idx)
            
			if subgraph.x.size(0) ==1:
				print(f'{node_idx} subgraph.x.size(0) ==1')
				pred_mask = torch.tensor([]).detach().cpu()
				new_data = Data(x=torch.tensor([0]), edge_index =torch.tensor([])) #안중요한 애들만 있음
				impt_data = Data(x=torch.tensor([0]), edge_index =torch.tensor([])) # 중요한 애들만 있음.
				subset = torch.tensor([node_idx])
				return subgraph, pred_mask, new_data, subset, impt_data
			else:
				node_mask, node_mask_in = self.__edge_mask_to_node__(subgraph, edge_mask, top_k) #안중요, 중요
				new_data = mask_fn(subgraph, node_mask)  # perturbed graph      #안중요한 애들만 있음           
				impt_data= mask_fn(subgraph, node_mask_in)   # 중요한 애들만 있음.   
				if 'Info' in str(self.model):
					with torch.no_grad():
						masked_embed, _, _ = self.model(new_data.x, new_data.edge_index)
				else:
					with torch.no_grad():
						#self.model.cpu()
						masked_embed  = self.model(new_data.x, new_data.edge_index)       
	...
	# 				'sparsity': sparsity_score}]
				#return subgraph, pred_mask, new_data, subset, impt_data #, related_preds
				return subgraph, pred_mask, new_data, subset, impt_data #, related_preds
          

def mask_fn(data: Data, node_mask: np.array): #mask값 1인 애들 data로 변환시켜줌.
	""" subgraph building through spliting the selected nodes from the original graph """
	row, col = data.edge_index
	edge_mask = (node_mask[row] == 1) & (node_mask[col] == 1) # which is less important edge
